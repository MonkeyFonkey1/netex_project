...
5. **Conturi, autorizare și modificarea contactelor (ziua 2).** Implementăm înregistrare, login, logout și sesiunea, cu roluri `USER` și `ADMIN`; înregistrarea publică nu acordă `ADMIN`. Adăugăm creare, editare, ștergere și validarea datelor în backend; păstrăm regulile în service, protejăm modificările pe server și verificăm autorul la editare/ștergere. Înlocuim paginile React de pregătire cu formulare asincrone. Verificare: utilizatorul autentificat poate crea și modifica propriile contacte, vizitatorul poate doar citi, iar modificarea contactelor altui autor este refuzată.
